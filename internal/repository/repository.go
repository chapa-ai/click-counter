...
func (r *counterRepository) GetStats(ctx context.Context, bannerID int, tsFrom time.Time, tsTo time.Time) ([]model.Stats, error) {
	rows, err := r.db.Query(ctx,
		"SELECT banner_id, COUNT(*), MIN(timestamp) FROM clicks WHERE banner_id = $1 AND timestamp BETWEEN $2 AND $3 GROUP BY banner_id",
	...
	for rows.Next() {
		...
		err = rows.Scan(&stat.BannerID, &stat.Count, &stat.Timestamp)
		if err != nil {
